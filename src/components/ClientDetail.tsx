...
import { Client, Staff, Task, TurnoverBracket, ClientTaskOverride, TaskArea, AiAnalysis } from '../types';
import { calculateClientProfitability } from '../services/calculator';
import { analyzeClientWithAI } from '../services/geminiService';
import { ArrowLeft, BrainCircuit, Activity, Building, University, Wallet, AlertCircle, CheckCircle, Phone, MapPin, FileText, Plus, Trash2, Save, User, Clock, Users, RefreshCcw, BadgeEuro } from 'lucide-react';
...
const ClientDetail: React.FC<ClientDetailProps> = ({ client, tasks, areaCosts, staff, turnoverBrackets, onBack, onUpdateClient }) => {
  ...
  const staffDistribution = useMemo(() => {
    ...
    // Initialize with 0
    staff.forEach(s => distribution[s.name] = 0);
    // Also add responsible staff if not in list (fallback)
    if (!distribution[responsibleStaffName]) distribution[responsibleStaffName] = 0;
    ...
    editedClient.tasks.forEach(ct => {
      ...
      if (task) {
        ...
        let staffName = responsibleStaffName;
        ...
        distribution[staffName] = (distribution[staffName] || 0) + hours;
      }
    });
    ...
    distribution[responsibleStaffName] = (distribution[responsibleStaffName] || 0) + opHours;

    // Filter out zero entries and sort
    return Object.entries(distribution)
      .filter(([_, hours]) => hours > 0)
      .sort((a, b) => b[1] - a[1]);
  }, [editedClient, tasks, staff, responsibleStaffName]);
                    ...
                    <table className="w-full text-sm text-left">
                      <thead className="text-xs text-slate-500 uppercase bg-slate-50">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-right">Custo/Ano</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-50">
                        {areaTasks.map((taskDef) => {
                          ...
                          if (ct?.assignedStaffId) {
                              ...
                              const responsible = staff.find(s => s.name === editedClient.responsibleStaff);
                              hourlyRate = responsible ? responsible.hourlyCost : (areaCosts[taskDef.area] || 25);
                          }

                          const currentMultiplier = ct?.multiplier || 0;
                      ...
                      {/* Operational Rows (Read Only) */}
                      {editedClient.callTimeBalance > 0 && (
                        <tr className="bg-slate-50/50 italic text-slate-500">
                          <td className="px-4 py-3 pl-8 text-xs">Apoio Telefónico ({editedClient.callTimeBalance} min/mês)</td>
                          <td className="px-4 py-3 text-center text-xs">12</td>
                          <td className="px-4 py-3 text-center text-xs">1</td>
                          <td className="px-4 py-3 text-xs">{editedClient.responsibleStaff}</td>
                          <td className="px-4 py-3 text-right text-xs">{((editedClient.callTimeBalance * 12)/60).toFixed(1)} h</td>
                      ...
                      {editedClient.travelCount > 0 && (
                        <tr className="bg-slate-50/50 italic text-slate-500">
                          <td className="px-4 py-3 pl-8 text-xs">Deslocações ({editedClient.travelCount}/ano)</td>
                          <td className="px-4 py-3 text-center text-xs">{editedClient.travelCount}</td>
                          <td className="px-4 py-3 text-center text-xs">1</td>
                          <td className="px-4 py-3 text-xs">{editedClient.responsibleStaff}</td>
                          <td className="px-4 py-3 text-right text-xs">{editedClient.travelCount.toFixed(1)} h</td>
                          <td className="px-4 py-3 text-right text-xs">-</td>
                          <td></td>
                        </tr>
                      )}
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
                ...
                <div className="space-y-3">
                   {staffDistribution.length > 0 ? (
                     staffDistribution.map(([name, hours]) => {
                       const percent = (hours / stats.totalAnnualHours) * 100;
                       return (
                         <div key={name}>
                           <div className="flex justify-between text-xs mb-1">
                             <span className="font-medium text-slate-700">{name}</span>
                             <span className="text-slate-500">{hours.toFixed(1)}h ({percent.toFixed(0)}%)</span>
                           </div>
