...
import { ArrowLeft, Save, RefreshCcw, User, Calculator, Layers, Briefcase, DollarSign, TrendingUp, Users, Activity } from 'lucide-react';
...
const StaffDetail: React.FC<StaffDetailProps> = ({ staffMember, clients, tasks, staff, areaCosts, onBack, onUpdateStaff }) => {
  ...
  const [activeTab, setActiveTab] = useState<'dados' | 'carteira'>('dados');
  ...
  const monthlyCost = stats.totalCost / 12;
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-10">
      ...
      {/* Leitura rápida: os números que interessam à primeira vista */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-100">
          <div className="flex items-center justify-between">
            <p className="text-xs font-bold text-slate-400 uppercase">Custo Mensal</p>
            <div className="p-1.5 bg-blue-50 rounded-lg text-blue-600"><DollarSign size={16} /></div>
          </div>
          <p className="text-3xl font-black text-slate-800 mt-1">{money.format(monthlyCost)}</p>
          <div className="mt-2 flex items-baseline justify-between border-t border-slate-100 pt-2">
            <span className="text-[11px] font-bold text-slate-400 uppercase">Custo / Hora</span>
            <span className="text-lg font-bold text-blue-600">{editedStaff.hourlyCost.toFixed(2)}€</span>
          </div>
        </div>
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-100">
          <div className="flex items-center justify-between">
            <p className="text-xs font-bold text-slate-400 uppercase">Nº de Empresas</p>
            <div className="p-1.5 bg-slate-100 rounded-lg text-slate-600"><Users size={16} /></div>
          </div>
          <p className="text-3xl font-black text-slate-800 mt-1">{stats.clientCount}</p>
        </div>
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-100">
          <div className="flex items-center justify-between">
            <p className="text-xs font-bold text-slate-400 uppercase">Rentabilidade</p>
            <div className={`p-1.5 rounded-lg ${stats.profitability < 20 ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'}`}><TrendingUp size={16} /></div>
          </div>
          <p className={`text-3xl font-black mt-1 ${stats.profitability < 20 ? 'text-red-500' : 'text-green-600'}`}>{stats.profitability.toFixed(0)}%</p>
        </div>
        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-100">
          <div className="flex items-center justify-between">
            <p className="text-xs font-bold text-slate-400 uppercase">Utilização</p>
            <div className="p-1.5 bg-amber-50 rounded-lg text-amber-600"><Activity size={16} /></div>
          </div>
          <p className="text-3xl font-black text-slate-800 mt-1">{stats.capacityUtilization.toFixed(0)}%</p>
          <div className="w-full bg-slate-100 rounded-full h-1.5 mt-2"><div className={`h-1.5 rounded-full ${stats.capacityUtilization > 90 ? 'bg-red-500' : stats.capacityUtilization > 70 ? 'bg-yellow-500' : 'bg-green-500'}`} style={{ width: `${Math.min(stats.capacityUtilization, 100)}%` }}></div></div>
        </div>
      </div>
      ...
      <div className="flex gap-1 border-b border-slate-200">
        <button
          ...
          className={`px-4 py-2.5 text-sm font-bold border-b-2 transition-colors ${activeTab === 'dados' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
        >
          Dados &amp; Custos
        </button>
        <button
          onClick={() => setActiveTab('carteira')}
          className={`px-4 py-2.5 text-sm font-bold border-b-2 transition-colors flex items-center gap-1.5 ${activeTab === 'carteira' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
        >
          Carteira de Clientes
          <span className="rounded-full bg-slate-100 px-1.5 py-0.5 text-[10px] text-slate-500">{stats.clientCount}</span>
        </button>
      </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select value={editedStaff.status || 'Ativo'} onChange={e => handleFieldChange('status', e.target.value)} className="w-full px-3 py-2 border rounded-lg text-sm bg-white">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
              ...
              <p className="mt-3 text-[11px] text-slate-400">
                Custo mensal médio já inclui subsídio de férias e de natal (salário ×14) e subsídio de alimentação só nos meses trabalhados (×11) — por isso o valor no topo da página é mais alto do que salário + TSU + subsídio somados uma vez.
              </p>
            </div>
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between text-sm"><span className="text-slate-500">Custo anual total:</span><span className="font-medium text-slate-700">{money.format(stats.totalCost)}</span></div>
                <div className="flex justify-between text-sm"><span className="text-slate-500">Avenças atribuídas (mês):</span><span className="font-medium text-slate-700">{money.format(stats.totalRevenue / 12)}</span></div>
              </div>
            </div>
          </div>
        </div>
